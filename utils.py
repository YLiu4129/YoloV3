...
def read_class_names(class_file_name):
    ...
    #param class_file_name: class 文件path
    names = {}
    with open(class_file_name, 'r') as data:
        #得到class name 以及label
...
def box_detector(pred):
    ...
    # change the shape of boxes to [top_left_y,top_left_x,bottom_right_y,bottom_right_x] for tf.image.non_max_suppression
    ...
    #get max probability of class vector
    class_index = np.argmax(classes, axis=-1)
    
    # the input shape of tf.image.non_max_suppression is [top_left_y,top_left_x,bottom_right_y,bottom_right_x]
